    ...
    - [x] 3.3 겹침 방지. 같은 워크플로우가 돌고 있으면 새 실행을 시작하지 않고 건너뛴다.
        ...
        - [x] 3.3.1 (추가) 겹침 방지를 두 진입점에 다 건다. 이미 있는 `POST /api/classify` 는
            ...
            - [x] 3.3.1.V 검증(제공 API): 부가 실행이 열려 있는 동안 `POST /api/classify` 가
                  ...
                  기록되는지 pytest
        - [x] 3.3.2 (수정) 겹침 방지가 "마지막 실행이 돌고 있는가" 를 보고 있었다. 건너뛴
              차례는 열자마자 닫힌 행이라, 한 번 건너뛰고 나면 그 행이 앞의 열린 실행을 가려
              그다음 차례부터 겹쳐 돌았다. 마지막 행이 아니라 열린 행을 찾도록 고쳤다
            - [x] 3.3.2.V 검증(스케줄러): 도는 중에 세 번째로 불러도 건너뛰기로 기록되는지
                  pytest
