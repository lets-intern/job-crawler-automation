...
- [ ] 3.0 실행기와 실행 기록
    ...
    - [ ] 3.1.1 (추가) 기동 시 `app/side/runs.py` 의 `close_orphans` 를 부른다. Push 1 이
          만들어 뒀지만 아무도 부르지 않아, 프로세스가 죽으며 남긴 열린 행이 영원히 진행
          중으로 남는다. 3.3 의 겹침 방지가 그 행을 보고 판단하므로 뒷정리가 없으면 죽은
          실행 하나가 워크플로우를 영구히 막는다
        - [ ] 3.1.1.V 검증(스키마): 열린 행을 남긴 채 기동해 `timeout` 으로 닫히는지 pytest
    - [ ] 3.2 `classify` 종류를 연결한다. `target_scope` 로 대상을 고르고 `batch_limit` 으로
          자른 뒤 `app/classify/batch.py` 에 넘긴다. 분류 자체의 코드는 고치지 않는다
        - [ ] 3.2.V 검증(정규화): 가짜 제공자로 픽스처 DB 를 돌려 처리·실패 건수가
              `side_runs` 에 그대로 들어가는지 pytest
    - [ ] 3.3 겹침 방지. 같은 워크플로우가 돌고 있으면 새 실행을 시작하지 않고 건너뛴다.
          건너뛴 사실을 로그와 `side_runs` 에 남긴다 — 조용히 사라지면 주기가 도는지 알 수 없다
        - [ ] 3.3.V 검증(스케줄러): 두 번 연달아 부르고 두 번째가 건너뛰기로 기록되는지 pytest
    - [ ] 3.4 실패 처리. 제공자 키가 없거나 호출이 전부 실패해도 실행은 닫히고 사유가
          `error_message` 에 남는다. 예외가 스케줄러까지 올라가지 않는다
        - [ ] 3.4.V 검증(스케줄러): 키 없는 상태로 돌려 실행이 `failed` 로 닫히는지 pytest
    - [ ] 3.5 `POST /api/side/{id}/run` 으로 한 번 돌린다. 응답은 시작했다는 것까지고
          진행은 `GET /api/side/{id}` 로 본다 (`app/api/classify.py` 와 같은 모양)
        - [ ] 3.5.V 검증(제공 API): 시작 요청이 202 이고, 도는 동안 GET 이 진행 중을
