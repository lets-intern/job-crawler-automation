...
- [x] 10.0 원문 보기
    - [x] 10.1 모달에 원문을 읽기 전용으로 붙인다. 고치는 칸이 아니다 — 원문은 수집한 것
          그대로여야 하고, `raw_jobs` 는 append-only 다 (`.claude/rules/data-safety.md`)
        - [x] 10.1.V 검증(화면): 모달을 열어 원문이 보이고 입력할 수 없는지 확인
    - [x] 10.2 원문이 없는 건에는 "이 건은 본문으로 분류됐다" 를 적는다. 빈 칸으로 두면
          원문을 못 뽑은 것과 아직 재수집하지 않은 것이 같아 보인다
        - [x] 10.2.V 검증(화면): 기존 수집분을 열어 그 문장이 나오는지 확인
    - [x] 10.3 표에는 열을 더하지 않는다. 이미 스물세 개다. 이 Push 가 `review_table.html` 의
          헤더를 건드리지 않았는지 확인한다
        - [x] 10.3.V 검증(화면): 검수 표를 열어 열 수가 그대로인지 확인
