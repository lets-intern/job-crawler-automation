...
- [ ] 23.0 목록·상세 수집 방식을 나눠 저장한다
    - [ ] 23.1 마이그레이션 0008: `crawlers` 에 `list_mode`, `detail_mode`, `api_config_json` 추가
        ...
        - [ ] 23.1.V 검증: 마이그레이션 적용·역적용 확인. 적용 후 크롤러 6개의 두 열이 이전
              `render_mode` 와 같은지 확인
    - [ ] 23.2 `api_config_json` 의 형식을 정하고 검증한다
        ...
        - [ ] 23.2.V 검증: 픽스처 기반 pytest — 형식에 맞는 설정은 통과하고, `items_path` 가
              없거나 `fields` 가 비면 이름을 대며 실패하는지
    - [ ] 23.3 API 로 목록·상세를 가져오는 경로를 만든다
        ...
        - 매칭 0건은 성공이 아니라 실패다
        - [ ] 23.3.V 검증: 저장한 LG 응답 픽스처로 pytest — 83건이 나오고 필드가 채워지는지.
              실서버를 때리지 않는다
    - [ ] 23.4 실행이 모드에 따라 갈리게 한다
