...
- [x] 11.0 채우기와 제안
    ...
    - [x] 11.6 검수 화면의 칸에 `제안 있음` 을 낱말로 적고, 제안 값과 이유를 보인다.
          수락하면 `job_field_overrides` 로 들어가고 거절하면 제안 행만 지운다. 어느 쪽이든
          `raw_jobs` 와 `normalized_jobs` 는 건드리지 않는다
        - [x] 11.6.V 검증(화면): 수락·거절을 각각 눌러 칸과 보정 개수가 갈리는지 확인
              (`tests/test_ui_review_suggestions.py`, TestClient 로 5개 통과. 로컬 서버는
              세션 쿠키 인증이 걸려 있고 자격증명이 없어 브라우저로는 열지 않았다)
    - [x] 11.7 조회 조건에 `제안이 있는 건` 을 더한다. 640건에서 제안이 붙은 것을 눈으로 찾게
          두면 아무도 수락하지 않는다
        - [x] 11.7.V 검증(화면): 조건을 걸어 제안 있는 건만 나오는지, 건수가 맞는지 확인
              (`tests/test_ui_review_suggestion_filter.py`, TestClient 로 5개 통과)
