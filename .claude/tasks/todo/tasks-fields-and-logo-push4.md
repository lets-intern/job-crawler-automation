...
- [x] 4.0 회사 표
    ...
    - [x] 4.3 정규화가 처음 보는 회사명을 만나면 행을 만든다. 로고는 비운다. 자동으로 만들지
          않으면 운영자가 회사명을 손으로 다시 치게 되고 오타 하나로 로고가 안 붙는다
        - [x] 4.3.V 검증(정규화): 같은 회사 공고 여러 건을 정규화해도 행이 하나인지, 자회사가
              빈 건은 모회사 이름으로 행을 만들지 확인하는 pytest
    - [x] 4.4 행을 자동으로 지우지 않는다. 공고가 다 사라진 회사도 남는다 — 지우는 것은
          운영자가 한다
        - [x] 4.4.V 검증(스키마): 그 회사의 공고를 다 지워도 `companies` 행이 남는지 pytest
    - [x] 4.5 재정규화도 행을 만든다. `app/normalize/backfill.py` 의 `rewrite_one` 은 UPDATE
          경로라 4.3 이 넣은 등록을 지나지 않는다. 이미 쌓인 공고는 재정규화가 유일한 등록
          경로이고, 규칙으로 회사명을 고친 뒤에도 새 이름의 행이 생겨야 한다
        - [x] 4.5.V 검증(정규화): 재정규화만으로 행이 생기는지, 이름을 바꾸는 규칙을 넣고 다시
              돌리면 새 이름의 행이 생기는지 pytest
