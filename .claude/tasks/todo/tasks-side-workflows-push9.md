...
**9.5 는 못 했다. 부를 수 있는 제공자가 없다.** 2026-08-28 에 픽스처 일곱 곳으로 본문 기준과
원문 기준을 각각 한 번씩 돌리려 했고 첫 호출에서 섰다.

| 제공자 | 키 | 결과 |
|---|---|---|
| Gemini | 있음 | 429 `Your prepayment credits are depleted` |
| Qwen | 있음 | 403 `AccessDenied.Unpurchased` |
| Claude | 없음 | 부를 수 없다 |
| GPT | 없음 | 부를 수 없다 |
| Ollama Cloud | 없음 | 키가 있어도 분류에는 못 쓴다. 응답을 스키마로 강제하지 못한다 |

크레딧이나 키가 들어오면 그대로 다시 돌린다. 대상은 상세가 HTML 인 일곱 곳의 픽스처이고,
한 곳당 본문 기준 1회 + 원문 기준 1회다. 볼 것은 칸별 채움과 버린 칸 수 둘이다.

측정을 대신할 수는 없지만 다음은 지금 안다. 원문이 본문보다 더 담은 글자는 SK 635자,
네이버 1,123자, 카카오 115자, 우아한형제들 128자, 롯데그룹 94자이고 두산 6자, 토스 2자다
(`.claude/site-recipes/source-text-container.md`). 앞의 다섯 곳에서 더 들어온 것이 이 공고의
회사·마감·근무지·경력 이름표 값이라, 채움이 늘어난다면 그 칸들이다.

**2026-08-29 재확인.** `classify_body` 를 실컨테이너에서 Gemini 로 한 번 더 불렀다. 여전히
같은 429 (`prepayment credits are depleted`) 다. 크레딧을 충전하거나 다른 제공자에 유효한
키를 넣기 전에는 9.5 를 시작할 수 없다 — 재시도만으로 풀리는 문제가 아니다. Push 11 은
Push 9 완료를 선행 조건으로 두므로 같이 막힌다.
