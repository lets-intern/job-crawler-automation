...
- [x] 9.0 분류 입력 교체
    ...
    - [x] 9.4 대상 조회의 "본문이 있다" 조건을 "원문이나 본문이 있다" 로 넓힌다.
        ...
        - [x] 9.4.V 검증(정규화): 원문만 있고 본문이 빈 건이 대상에 들어오는지 pytest
              ...
              원문도 본문도 없는 건은 빠진다
    - [x] 9.5 실측 1회. 원문이 있는 건 몇 개를 실제 제공자로 돌려 열한 칸이 전보다 더
          채워지는지 본다. 채워진 칸 수와 버린 칸 수를 결과보고서에 적는다
        - [x] 9.5.V 검증(셀렉터 생성): 같은 공고를 본문 기준과 원문 기준으로 각각 분류해
              칸별 채움을 비교한 표를 남긴다

**2026-08-28 실측 완료.** Gemini 크레딧이 반영된 뒤 상세가 HTML 인 일곱 곳의 픽스처로 본문
기준·원문 기준을 각각 한 번씩(`gemini-3.5-flash`, 실제 호출 14회) 돌렸다. 근거 검사가 버린
칸은 일곱 곳 전부 0건이다 — 원문을 보냈다고 근거 없는 값이 늘어나지는 않았다.

| 사이트 | 본문 글자수 | 원문 글자수 | 본문 채움 | 원문 채움 | 원문에서 늘어난 칸 | 원문에서 줄어든 칸 |
|---|---|---|---|---|---|---|
| SK | 2,357 | 2,992 | 8 | 7 | employment_type | hiring_process, preferred |
| 롯데그룹 | 1,404 | 1,498 | 9 | 9 | 없음 | 없음 |
| 두산 | 1,974 | 1,980 | 5 | 6 | etc_info | 없음 |
| 네이버 | 2,749 | 3,872 | 4 | 8 | career_level, etc_info, hiring_process, preferred | 없음 |
| 토스 | 10,310 | 10,312 | 7 | 5 | 없음 | hiring_process, preferred |
| 카카오 | 1,357 | 1,472 | 8 | 9 | work_location | 없음 |
| 우아한형제들 | 198 | 326 | 5 | 5 | 없음 | 없음 |

일곱 곳 합계는 본문 기준 46칸, 원문 기준 49칸으로 원문이 3칸 더 채웠다. 네이버(+4)·두산(+1)·
카카오(+1)가 늘었고, 롯데그룹·우아한형제들은 그대로다.

SK(-1)와 토스(-2)는 원문 기준에서 오히려 줄었다. 이 둘은 원문이 본문보다 늘어난 글자가
각각 635자·2자뿐이고, 그 늘어난 글자는 지원기간·마감시간처럼 이미 다른 칸으로 뽑히는
이름표 값이라 `hiring_process`·`preferred` 를 채우는 데 도움이 될 내용이 아니다 — 늘어난
글자 자체가 원인이라기보다, **한 번만 돌린 호출의 응답 변동**일 가능성이 높다. 두 조건 모두
같은 프롬프트 구조에 같은 모델을 1회씩만 부른 것이라, 이 차이가 원문 탓인지 모델의
호출 간 변동인지는 이번 측정으로는 가르지 못한다. 여러 번 돌려 평균을 보는 것은 이 Push의
범위를 넘는다.

전체적으로 원문 기준이 본문 기준보다 나쁘지 않고(근거 없는 값이 늘지 않았고, 합계도 늘었다),
5.1(`app/classify/store.py::read_source`)가 이미 들어가 있으므로 되돌릴 이유는 없다.
