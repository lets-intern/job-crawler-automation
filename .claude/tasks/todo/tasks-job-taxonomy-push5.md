...
- [ ] 5.0 검수 화면과 제공 API
    - [ ] 5.1 검수 화면 표에 `직무 대분류`, `직무 소분류` 두 칸을 더한다. `job_role`(제목에서
          뽑은 자유 텍스트) 칸 옆에 둔다 — 이름이 비슷해 혼동하지 않도록 라벨을 분명히
          가른다("직무"(자유 텍스트) vs "직무 대분류/소분류"(닫힌 목록))
        - [ ] 5.1.V 검증(화면): 로컬에서 검수 화면을 열고 두 칸이 보이는지, 값이 있는 건과
              없는 건이 구분되는지 확인
    - [ ] 5.2 조회 조건에 직무 대분류 필터를 더한다(소분류는 대분류에 종속되므로 대분류
          하나만 먼저 둔다). 목록은 `job_taxonomy`의 켜진 대분류에서 읽는다
        - [ ] 5.2.V 검증(화면): 대분류를 골라 조회하면 그 값을 가진 건만 나오는지 확인
