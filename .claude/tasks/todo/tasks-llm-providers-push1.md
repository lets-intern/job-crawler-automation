...
- [ ] 1.0 호출 자리를 제공자에서 떼어내고 넷을 붙인다
    ...
    - [x] 1.7 호출 기록에 실제 제공자가 남는다
        - `app/llm/log.py` 가 `from app.llm.gemini import PROVIDER` 로 상수를 박고 있다.
          호출한 제공자를 넘겨받도록 고친다
        - **마이그레이션은 만들지 않는다.** `llm_calls.provider` 칸은 0013 에 이미 있다
        - 실행 중 정한 것: 제공자 이름을 `record_call` 의 인자로 따로 받지 않고 `Usage` 에
          실었다. 답한 제공자만이 그 값을 알고, 기록하는 쪽에서 설정을 다시 읽어 알아내면
          호출과 기록 사이에 설정이 바뀌었을 때 기록이 거짓이 된다. `Usage` 를 만드는
          테스트 여덟 곳도 같이 고쳤다
        - [x] 1.7.V 검증: `tests/test_llm_calls.py` 를 고쳐, 서로 다른 제공자로 두 번 기록한 뒤
              `provider` 로 갈라 세었을 때 각각 맞게 나오는지 확인한다

    - [ ] 1.8 Qwen 으로 실제 분류를 돌린다 — **막혔다. 코드가 아니라 계정과 데이터다**
        ...
        2026-08-27 에 막힌 것 둘. 둘 다 이 저장소에서 풀 수 있는 것이 아니다.

        | 막은 것 | 확인한 방법 | 풀리는 조건 |
        |---|---|---|
        | Qwen 계정에 모델 이용 자격이 없다 | 실제 호출이 403 `AccessDenied.Unpurchased`. 없는 모델은 404 로 갈리므로 이름 문제가 아니다 | 콘솔에서 Model Studio 활성화 또는 결제 수단 등록 |
        | 이 기계에 운영 데이터가 없다 | `DATABASE_PATH` 인 `./data/jobs.db` 가 0바이트이고 표가 하나도 없다 | 배포 서버에서 돌리거나 스냅샷을 받아 온다 |

        Gemini 로 대신 돌릴 수도 없다. 크레딧이 소진돼 429 다. Claude 와 GPT 는 키가 없다.
        **지금 부를 수 있는 제공자가 하나도 없다.**

        여기까지는 확인했다. 실제 키로 `classify_body()` 를 한 번 불러, 설정에서 고른
        제공자가 Qwen 항목으로 이어지고 DashScope 에 실제 요청이 나가고 그 실패가
        `ClassifyError("api_error", "Qwen 호출 실패(403): ...")` 로 돌아오는 것까지 봤다.
        **배선은 끝났고 남은 것은 자격과 데이터뿐이다.**

        다음에 할 일은 그대로다 — 자격이 풀린 서버에서 3건을 먼저 돌리고 눈으로 본 뒤
        나머지를 돌린다. 운영 DB 에 쓰는 작업이라 3건 확인 전에 전체를 돌리지 않는다.
