...
- [x] 2.0 열한 사이트 매핑을 다시 쓴다
    ...
    - [x] 2.5 레시피에 매핑을 적는다
        - 열한 파일에 `## 칸 매핑 (2026-08-26, 본문 나누기 Push 2)` 절을 더했다. 칸 열여섯 개를
          한 줄씩 적고, 어디서 읽는지(상세 API / 상세 HTML / 목록 API)와 자리를 함께 적는다
        - **필드명은 설정에서 프로그램으로 복사했다.** 기억으로 다시 쓰지 않았다
        - 삼성처럼 상세가 API 인 사이트는 `selectors_json` 이 남아 있어도 적지 않는다. 안 쓰는
          값을 문서에 적으면 다음 사람이 그것을 고친다
        - 카카오 목록 설정에서 `body`·`employment_type`·`work_location` 을 뺐다. 상세 문서가
          이름표로 주는 칸이라 상세가 이기고, 두 자리에 같은 칸이 적혀 있으면 어디서 온 값인지
          문서로 설명할 수 없다
        - [x] 2.5.V 검증: `tests/test_site_recipe_mapping.py` 34개 통과. 열한 레시피에 절이
          다 있는지와, 문서에 적힌 자리와 설정의 자리가 **양쪽 모두** 빠짐없이 같은지를 본다
...
## 사이트별로 채워지는 칸 (2026-08-26)

픽스처에 설정을 그대로 돌린 결과다. 세는 것은 `tests/test_split_body_mapping.py` 가 들고 있다.
`제목` 과 `본문 원문` 은 열한 곳 다 채우므로 표에서 뺐다 — 비면 수집 자체가 실패다.

| 칸 | LG | 한화 | 삼성 | 현대 | SK | 롯데 | 두산 | 네이버 | 토스 | 카카오 | 우아한 |
|---|---|---|---|---|---|---|---|---|---|---|---|
| 필수 조건 | 채움 | 채움 | 채움 | 채움 | 채움 | 채움 | 채움 | 비움 | 비움 | 채움 | 비움 |
| 모집 마감일 | 채움 | 채움 | 목록 | 채움 | 채움 | 채움 | 채움 | 채움 | 채움 | 채움 | 채움 |
| 조직·부서 | 채움 | 비움 | 비움 | 채움 | 비움 | 비움 | 비움 | 비움 | 비움 | 비움 | 비움 |
| 모집 기업 | 채움 | 채움 | 채움 | 운영자 | 채움 | 채움 | 채움 | 채움 | 비움 | 채움 | 비움 |
| 모집 시작일 | 채움 | 채움 | 채움 | 채움 | 채움 | 채움 | 채움 | 채움 | 비움 | 채움 | 채움 |
| 직군 | 채움 | 비움 | 비움 | 채움 | 채움 | 비움 | 채움 | 채움 | 비움 | 채움 | 채움 |
| 고용형태 | 비움 | 비움 | 비움 | 비움 | 채움 | 비움 | 비움 | 채움 | 비움 | 채움 | 채움 |
| 경력 구분 | 채움 | 비움 | 비움 | 채움 | 채움 | 비움 | 비움 | 채움 | 비움 | 비움 | 채움 |
| 근무지 | 채움 | 채움 | 채움 | 채움 | 채움 | 비움 | 채움 | 비움 | 비움 | 채움 | 비움 |
| 모집인원 | 채움 | 채움 | 비움 | 비움 | 비움 | 비움 | 채움 | 비움 | 비움 | 채움 | 비움 |
| 주요 업무 | 채움 | 채움 | 채움 | 채움 | 채움 | 비움 | 채움 | 비움 | 비움 | 채움 | 비움 |
| 우대 조건 | 채움 | 비움 | 채움 | 채움 | 채움 | 비움 | 비움 | 비움 | 비움 | 비움 | 비움 |
| 전형 절차 | 채움 | 채움 | 비움 | 채움 | 채움 | 채움 | 채움 | 비움 | 비움 | 채움 | 비움 |
| 기타 | 채움 | 채움 | 채움 | 채움 | 채움 | 채움 | 채움 | 비움 | 비움 | 채움 | 비움 |

`목록` 은 상세가 아니라 목록 응답에서 온다는 뜻이다. `운영자` 는 `crawlers.default_company`
에 적어 둔 값이다.

## 아직 안 한 것

- **기존 321건은 새 칸이 전부 NULL 이다.** 그 건들의 `raw_jobs.raw_data_json` 에 옛 여섯 칸만
  들어 있어서, 재정규화를 돌려도 새 칸에 넣을 값이 없다. 채우는 것은 재수집이고 Push 3 이다
- API 설정을 고치는 **화면**은 없다. 라우트(`PUT /api/crawlers/{id}/api-config`)만 있고
  셀렉터 편집기 같은 편집 화면은 만들지 않았다
- HTML 상세는 칸 하나에 셀렉터 하나다. API 처럼 여러 자리를 모을 수 없어, SK 마감 시간
  (`23:59`)처럼 본문 밖에 있고 칸이 없는 값 몇 개는 어디에도 안 들어간다
- 실사이트 요청을 하지 않았다. 확인은 전부 픽스처와 운영 DB 로 했다
