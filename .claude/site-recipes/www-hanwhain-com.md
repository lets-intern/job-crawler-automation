...
- 수집 방식: 목록·상세 둘 다 `api` (2026-08-25 등록, crawler 19 / workflow 7). 브라우저를
  띄우지 않는다
- 페이지네이션: 본문에 `page` 를 더한다. 0부터 20건씩이고 마지막 쪽에서 `data.hasNext` 가
  false 가 된다. 2026-08-25 에 4쪽 68건이었다. `totalCount` 는 첫 쪽에만 온다
- 날짜 포맷: 목록·상세 모두 `2026.08.25 15:00`. 마감 일시 하나이고 기간이 아니다.
  `deadline` 규칙이 시각을 떼고 `%Y.%m.%d` 로 읽는다
- robots.txt: **두 호스트 모두 robots.txt 가 없다.** 200 을 주지만 내용이 SPA 셸 HTML 이다.
  제한이 명시되지 않은 것이지 넉넉히 다녀도 된다는 뜻이 아니라서 딜레이를 그대로 지킨다
- 권장 주기: 360분으로 등록했다 (workflow 7)

## 2026-08-22 기록: 그때는 등록할 수 없었다

아래 두 절은 렌더 경로만 보던 시점의 판정이다. 2026-08-24 에 뒤집혔다.
...
## 2026-08-25: 등록했다. 68건이 들어온다

목록·상세 API 를 그대로 쓴다. 요청 전문과 필드 매핑은 `seeds/site-configs-20260826.json` 의
한화 항목에 있고 DB 가 진실이다.

첫 실행(run 248)은 3분 37초에 68건을 적재했고 실패 0건, 본문이 빈 행 0건이다. 계열사는
`sdNm` 에서 오므로 `default_company` 를 쓰지 않는다.

본문은 `data.item.unitDt` 의 모집 단위마다 있는 `ruNm`·`ruDtlJob` 에 `rtEct`(기타)를 더해
모은다. 자격요건은 `rtExmQlf` 다.

### API 가 본문을 거의 주지 않는 공고가 있다

68건 중 6건은 `unitDt` 가 비어 있거나 직무명만 있어서 본문이 20자 미만이다 (100자 미만은 13건). 가장 짧은 것은
`rtEct` 가 문자열 `"0"` 하나인 공고(rtSeq 19382)다. 응답에 실제로 그렇게 들어 있고 파싱
문제가 아니다 — 내용이 첨부파일에 있는 공고로 보인다.
