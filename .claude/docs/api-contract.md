...
**2026-08-26 이전에 수집된 행은 아직 예외다.** 그때는 이 세 값을 사이트에서 그대로 받아
적었고, `raw_jobs` 는 append-only 라 그 값이 남아 있다. 정규화는 수집이 준 값을 분류보다
먼저 두므로, 그런 행은 `Permanent` 나 `IT - 구축/운영/최적화` 처럼 사이트의 표기가 그대로
나간다 (2026-08-26 기준 `job_category` 269건, `employment_type` 131건, `career_level`
230건). 소비 측이 이 세 필드로 거르려면 그 행들을 어떻게 할지 먼저 정해야 한다.
