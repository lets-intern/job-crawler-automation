...
## 여섯 칸은 수집이, 여덟 칸은 분류가 가진다

수집은 어느 사이트나 확실히 주는 여섯 칸만 한다 — `title` `body` `company` `deadline`
`start_date` `source_url`. 나머지 여덟 칸은 본문을 읽어 나눈 결과가 채운다 (`app/classify/`).
그 결과는 `job_classifications` 에 따로 남아 있다.

**분류가 있으면 그 여덟 칸은 전부 분류 값이다.** 규칙이 만든 값이 있어도 덮고, 분류가 빈 칸을
...
def apply_classification(
    ...
    """여덟 칸을 분류 결과로 바꾼다. 규칙이 만든 값이 있어도 덮고, 빈 칸이면 비운다.
